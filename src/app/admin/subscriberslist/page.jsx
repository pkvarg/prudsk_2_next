    ...
    <div className="min-h-screen px-4 sm:px-6 py-6 sm:py-8">
      <div className="container mx-auto max-w-4xl">
        ...
        <div className="bg-white rounded-lg shadow-md p-4 sm:p-6 mb-8">
          <div className="space-y-4">
            {subscribers?.map((user) => (
              <div key={user.id} className="border-b border-gray-200 pb-3 last:border-b-0">
                {/* Name over e-mail on mobile — long addresses used to run off-screen */}
                <p className="text-[#9b7d57] text-base sm:text-lg">
                  <span className="font-medium block sm:inline break-words">{user.name}</span>
                  <span className="mx-2 text-gray-400 hidden sm:inline">•</span>
                  <span className="text-gray-600 block sm:inline text-sm sm:text-base break-all">
                    {user.email}
                  </span>
                </p>
              </div>
          ...
          <button
            onClick={copyEmails}
            className="w-full sm:w-auto mt-6 bg-[#071e46] text-white px-6 py-3 rounded-lg font-medium hover:bg-[#0a2554] transition-colors duration-200 focus:outline-none focus:ring-2 focus:ring-[#071e46] focus:ring-offset-2 cursor-pointer"
          >
        ...
        <div className="bg-white rounded-lg shadow-md p-4 sm:p-6">
          ...
          <div className="space-y-4">
            {unsubscribers?.map((user) => (
              <div key={user.id} className="border-b border-gray-200 pb-3 last:border-b-0">
                <p className="text-[#9b7d57] text-base sm:text-lg opacity-75">
                  <span className="font-medium block sm:inline break-words">{user.name}</span>
                  <span className="mx-2 text-gray-400 hidden sm:inline">•</span>
                  <span className="text-gray-600 block sm:inline text-sm sm:text-base break-all">
                    {user.email}
